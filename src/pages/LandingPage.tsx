...
import teamJongho from "../assets/team_jongho.png";
import teamSiyeon from "../assets/team_siyeon.png";
...
function OdmLogo({ variant = "nav" }: { variant?: "nav" | "hero" | "footer" }) {
  return (
    <img
      src={odmLogo}
      alt="ODM"
...
function LineIcon({ name, className = "" }: { name: keyof typeof iconPaths; className?: string }) {
  return (
...
type GlowButtonProps = {
  children: ReactNode;
  href?: string;
  variant?: "primary" | "secondary" | "cream" | "quiet";
  size?: "cta" | "compact";
};

function GlowButton({ children, href = "#beta", variant = "primary", size = "cta" }: GlowButtonProps) {
  const navigate = useNavigate();
  const styles =
    variant === "primary"
      ? "cta-button-primary bg-[linear-gradient(135deg,#3a251d_0%,#81543b_48%,#d7ad72_100%)] text-white"
      : variant === "secondary"
      ...
      : variant === "cream"
        ...
        : "cta-button-quiet border border-coffee/15 bg-white/54 text-espresso shadow-warm backdrop-blur-xl";
  const sizing =
    size === "compact"
  ...
  if (href.startsWith("/")) {
    return (
      <motion.button type="button" onClick={() => navigate(href)} className={className} {...motionProps}>
        {children}
      </motion.button>
    );
  }
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        <nav className="flex items-center justify-between gap-3 rounded-full border border-coffee/10 bg-ivory/82 px-4 py-3 shadow-warm backdrop-blur-2xl md:px-6">
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            <a
              ...
              className="hidden min-h-[3.35rem] items-center justify-center rounded-full border border-coffee/15 bg-white/54 px-5 text-sm font-bold text-espresso shadow-warm transition hover:-translate-y-0.5 hover:bg-ivory focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caramel/55 focus-visible:ring-offset-2 focus-visible:ring-offset-cream sm:inline-flex"
            >
              GitHub
            </a>
            <button
              type="button"
              aria-controls="navbar-team-portraits"
              aria-expanded={isTeamOpen}
              onClick={() => setIsTeamOpen((open) => !open)}
              className={`inline-flex min-h-[3.35rem] items-center justify-center rounded-full border px-5 text-sm font-bold shadow-warm transition duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-caramel/55 focus-visible:ring-offset-2 focus-visible:ring-offset-cream ${
                isTeamOpen
                  ? "border-caramel/30 bg-linen/70 text-espresso"
                  : "border-coffee/15 bg-white/54 text-coffee hover:-translate-y-0.5 hover:bg-ivory hover:text-espresso"
              }`}
            >
              팀소개
            </button>
        ...
        <motion.div
          id="navbar-team-portraits"
          aria-hidden={!isTeamOpen}
          initial={false}
          animate={{
            height: isTeamOpen ? "auto" : 0,
            opacity: isTeamOpen ? 1 : 0,
          }}
          transition={{ duration: 0.5, ease: "easeInOut" }}
          className={`overflow-hidden ${isTeamOpen ? "" : "pointer-events-none"}`}
        >
          <motion.div
            animate={{
              opacity: isTeamOpen ? 1 : 0,
              y: isTeamOpen ? 0 : 20,
              scale: isTeamOpen ? 1 : 0.95,
            }}
            transition={{ duration: 0.5, ease: "easeInOut" }}
            className="origin-top px-1 pt-4"
          >
            <div className="rounded-2xl border border-coffee/10 bg-ivory/90 p-4 shadow-soft backdrop-blur-2xl sm:p-5">
              <div className="grid gap-4 md:grid-cols-2 md:gap-5">
                {teamPortraits.map((member) => (
                  <motion.figure
                    key={member.name}
                    whileHover={{ y: -6, scale: 1.01 }}
                    transition={{ duration: 0.3, ease: "easeOut" }}
                    className="flex min-h-[15rem] flex-col items-center justify-end overflow-hidden rounded-2xl bg-paper/80 p-4 shadow-warm md:min-h-[18rem]"
                  >
                    <div className="flex w-full flex-1 items-end justify-center">
                      <img
                        src={member.image}
                        alt={`${member.name} 팀원 캐릭터`}
                        className="h-auto max-h-[22rem] w-full max-w-[19rem] object-contain drop-shadow-[0_18px_32px_rgba(83,58,42,.18)]"
                        draggable="false"
                      />
                    </div>
                    <motion.figcaption
                      ...
                      transition={{ duration: 0.5, ease: "easeInOut" }}
                      className="mt-4 w-full rounded-2xl border border-coffee/8 bg-ivory/68 px-4 py-3 text-center shadow-warm"
                    >
                      <span className="block text-sm font-semibold text-coffee/72">{member.name}</span>
                      <span className="mt-1 block text-sm font-medium text-brown/72">{member.role}</span>
                    </motion.figcaption>
                  </motion.figure>
                ))}
              </div>
    ...
    <section className="hero-section relative flex min-h-[92svh] items-center overflow-hidden px-4 pb-16 pt-28 md:pt-32">
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="text-center lg:text-left">
          ...
          <motion.div
            className="cta-group mt-8 flex flex-col justify-center gap-4 sm:flex-row sm:flex-wrap lg:justify-start"
      ...
      <div className="mx-auto mt-14 grid max-w-6xl gap-5 md:grid-cols-2 lg:grid-cols-4">
        {serviceCards.map((card, index) => (
          <Reveal key={card.title} delay={index * 0.08}>
            <motion.article className="lift-card h-full rounded-[1.6rem] border border-coffee/10 bg-ivory/72 p-6 shadow-premium backdrop-blur-xl">
      ...
      <div className="mx-auto mt-14 grid max-w-6xl gap-5 md:grid-cols-2 lg:grid-cols-3">
        {featureCards.map((feature, index) => (
          <Reveal key={feature.title} delay={index * 0.06}>
            <motion.article className="feature-card h-full rounded-[1.8rem] border border-coffee/10 bg-ivory/76 p-6 shadow-premium backdrop-blur-xl">
...
function PreviewScreen({ screen, index }: { screen: (typeof previewScreens)[number]; index: number }) {
  const slideOffset = index % 2 === 0 ? -20 : 20;

  return (
    <motion.div
      initial={{ opacity: 0, x: slideOffset, y: 26 }}
      whileInView={{ opacity: 1, x: 0, y: 0 }}
      viewport={{ once: true, margin: "-90px" }}
      transition={{ duration: 0.9, delay: index * 0.08, ease: [0.16, 1, 0.3, 1] }}
    >
      <motion.article
        className="mockup-card rounded-[1.8rem] border border-coffee/10 bg-ivory/78 p-3 shadow-premium backdrop-blur-2xl"
...
function VideoPreviewPlaceholder() {
  return (
    <section className="section-band px-4 py-24 md:py-28">
      <Reveal className="mx-auto flex max-w-6xl justify-center" y={24}>
        <video
          className="aspect-video w-full max-w-[900px] rounded-[2rem] border border-coffee/10 bg-paper object-cover shadow-[0_24px_58px_rgba(83,58,42,.18)]"
          src="/videos/odm-preview.mp4"
          controls
          autoPlay
          muted
          loop
          playsInline
          aria-label="ODM Preview Video"
        />
      </Reveal>
    </section>
  );
      ...
      <Reveal>
        <div className="beta-panel mx-auto max-w-6xl overflow-hidden rounded-[2.4rem] border border-white/10 bg-espresso px-6 py-16 text-center text-cream shadow-premium md:px-12 md:py-24">
          ...
          <div className="cta-group mt-10 flex flex-col justify-center gap-5 sm:flex-row">
            <GlowButton href="mailto:hello.psyeon@gmail.com?subject=ODM 출시 알림 신청">
              출시 알림 받기
            </GlowButton>
            <GlowButton href="mailto:hello.psyeon@gmail.com?subject=ODM 베타테스터 신청" variant="secondary">
              베타테스터 신청
            </GlowButton>
          </div>
        </div>
...
const teamPortraits = [
  { name: "시연", role: "기획 및 백엔드 개발", image: teamSiyeon },
  { name: "종호", role: "디자인 및 프론트", image: teamJongho },
];
