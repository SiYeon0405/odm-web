...
import { getClubReviews, getReviewErrorMessage, type Review, type ReviewPage } from "@/features/reviews/api/reviewsApi";

const PAGE_SIZE = 20;

export default function ClubReviewListPage() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setErrorMessage("");
    getClubReviews(id, page, PAGE_SIZE)
      .then((data) => {
        if (!active) return;
        setReviewsPage(data);
      })
      ...
      <main className="home-page min-h-screen bg-cream font-sans text-espresso">
        ...
        <section className="relative mx-auto max-w-5xl px-4 pb-20 pt-40 md:pt-48">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              type="button"
              onClick={() => navigate(`/clubs/${clubId}/reviews/new`)}
              disabled={!clubId}
          ...
          {!isLoading && !errorMessage && reviews.length === 0 && (
            <p className="mt-8 text-coffee/64">등록된 독후감이 없습니다.</p>
          )}
          {!isLoading && !errorMessage && reviews.length > 0 && (
